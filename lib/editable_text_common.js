...
EditableText._allowedHtml = function (maxImageSize) {
  ...
  if (maxImageSize && _.isNumber(maxImageSize) && allowed) {
    ...
    allowed.allowedAttributes.img = _.union(allowed.allowedAttributes.img || [], ['src', 'alt', 'title']);
        ...
        case 'update' :
          if (Meteor.isServer) {
            ...
            if (modifier["$set"] && _.isArray(modifier["$set"][data.field])) {
              newValue = modifier["$set"][data.field] = _.map(modifier["$set"][data.field], function (str) {
                // allow for an array of objects (coming from editableList)
                if (data.key && _.isObject(str)) { // sanitize all string fields here, but who knows what else is in the object -- devs using this package should also use simpleSchema to keep this secure
                  _.each(str, function (v, k) {
                    if (_.isString(v)) {
                      str[k] = EditableText.sanitizeString.call(data, v, data.wysiwyg || !!data.editor);
                    }
                  });
                  return str;
                }
                return EditableText.sanitizeString.call(data, str, data.wysiwyg || !!data.editor);
              });
              sanitized = true;
            }
