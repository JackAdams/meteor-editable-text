if (typeof EditableText === 'undefined') {
  EditableText = {};
}


// ******
// CONFIG
// ******

EditableText.userCanEdit = function() {
  return (typeof this.userCanEdit !== 'undefined') ? this.userCanEdit : true;	
}

EditableText.useTransactions = (typeof tx !== 'undefined' && _.isObject(tx.Transactions)) ? true : false;



// *********************************
// INTERNAL PROPERTIES AND FUNCTIONS
// *********************************

EditableText._callback = function(callback,doc) {
  var self = this;
  if (self[callback] && _.isString(self[callback])) {
    var mutatedDoc = EditableText._executeCallback(self[callback],self,doc);
    doc = _.isObject(mutatedDoc) && mutatedDoc || doc;
  }
  return doc;
}

EditableText._executeCallback = function(callbackFunctionName,self,doc) {
  var mutatedDoc = doc;
  var callbackFunction = EditableText._drillDown((Meteor.isServer) ? global : window,callbackFunctionName);
  if (callbackFunction && _.isFunction(callbackFunction)) {
	mutatedDoc = callbackFunction.call(self,doc,Mongo.Collection.get(self.collection)) || mutatedDoc;
  }
  else {
	throw new Meteor.Error('Could not execute callback. Reason: ' + ((callbackFunction) ? '"' + callbackFunctionName + '" is not a function, it\'s a ' + typeof(callbackFunction) + '.' : 'no callback function called "' + callbackFunctionName + '" defined in the global scope.'));	
  }
  return mutatedDoc;
}

EditableText._drillDown = function(obj,key) {
  var pieces = key.split('.');
  if (pieces.length > 1) {
	var newObj = obj ? obj[pieces[0]] : {};
	pieces.shift();
	return EditableText._drillDown(newObj,pieces.join('.'));
  }
  else {
	if (obj) {
	  return obj[key];
	}
	else {
	  return; // undefined	
	}	
  }
}


// **************
// UPDATE METHODS
// **************

Meteor.methods({
  editableTextWrite : function(action,data,modifier,transaction) {
	check(action,String);
	check(data.collection,String);
	check(data.context,Object);
	check(modifier,(action === 'update') ? Object : null);
	check(transaction,Boolean);
	check(data.objectTypeText,Match.OneOf(String,undefined));
	var Collection = Mongo.Collection.get(data.collection);
	if (EditableText.userCanEdit.call(data,Collection)) {
	  if (Meteor.isServer) {
  	    var allowed = EditableText.allowedHtml;
		if (_.isBoolean(EditableText.useTransactions)) {
		  transaction = EditableText.useTransactions;	
		}
	  }
	  switch (action) {
		case 'insert' :
		  if (Meteor.isServer) {
		    // run all string fields through sanitizeHtml
			data.context = EditableText.sanitizeObject(data.context);
		  }
		  if (transaction) {
			if (data.objectTypeText) {
			  tx.start('add ' + data.objectTypeText);
			}
			data.context = Meteor.isServer && EditableText._callback.call(data,'beforeInsert',data.context) || data.context;
			var new_id = tx.insert(Collection,data.context,{instant:true});
			if (Meteor.isServer) {
			  EditableText._callback.call(data,'afterInsert',data.context);
			}
			if (data.objectTypeText) {
			  tx.commit();
			} 
		  }
		  else {
			data.context = Meteor.isServer && EditableText._callback.call(data,'beforeInsert',data.context) || data.context;
			var new_id = Collection.insert(data.context);
			if (Meteor.isServer) {
			  EditableText._callback.call(data,'afterInsert',data.context);
			}
		  }
		  return new_id;
		  break;
		case 'update' :
		  if (Meteor.isServer && _.isString(modifier["$set"][data.field])) {
		    // run through sanitizeHtml
		    modifier["$set"][data.field] = sanitizeHtml(modifier["$set"][data.field],allowed);
		  }
		  if (transaction) {
			if (data.objectTypeText) {
			  tx.start('update ' + data.objectTypeText);
			}
			data.context = Meteor.isServer && EditableText._callback.call(data,'beforeUpdate',data.context) || data.context;
			tx.update(Collection,data.context._id,modifier,{instant:true});
			if (Meteor.isServer) {
			  EditableText._callback.call(data,'afterUpdate',data.context);
			}
			if (data.objectTypeText) {
			  tx.commit();
			}
		  }
		  else {
			data.context = Meteor.isServer && EditableText._callback.call(data,'beforeUpdate',data.context) || data.context;
			Collection.update({_id:data.context._id},modifier);
			if (Meteor.isServer) {
			  EditableText._callback.call(data,'afterUpdate',data.context);
			}
		  }
		  break;
		case 'remove' :
		  if (transaction) {
			if (data.objectTypeText) {
			  tx.start('remove ' + data.objectTypeText);
			}
			if (Meteor.isServer) {
			  EditableText._callback.call(data,'afterUpdate',data.context);
			}
			data.context = Meteor.isServer && EditableText._callback.call(data,'beforeRemove',data.context) || data.context;
			tx.remove(Collection,data.context._id,{instant:true});
			if (Meteor.isServer) {
			  EditableText._callback.call(data,'afterRemove',data.context);
			}
			if (data.objectTypeText) {
			  tx.commit();
			} 
		  }
		  else {
			data.context = Meteor.isServer && EditableText._callback.call(data,'beforeRemove',data.context) || data.context;
			Collection.remove({_id:data.context._id});
			if (Meteor.isServer) {
			  EditableText._callback.call(data,'afterRemove',data.context);
			}
		  }
		  break;  
	  }
	}
  }
});