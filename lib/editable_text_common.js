      ...
      switch (action) {
        ...
        case 'update' :
          if (Meteor.isServer) {
            ...
            newValue = (modifier["$set"] && (modifier["$set"][data.field] || (data.type === 'number' && modifier["$set"][data.field] === 0))) || (modifier["$addToSet"] && modifier["$addToSet"][data.field]) || (modifier["$push"] && modifier["$push"][data.field]);
          }
