      ...
      switch (action) {
        ...
        case 'update' :
          if (Meteor.isServer) {
            ...
            newValue = modifier["$set"][data.field] || modifier["$addToSet"][data.field] || modifier["$push"][data.field];
          }
          ...
          if (transaction) {
            ...
            data.context = EditableText._callback.call('beforeUpdate',data.context) || data.context;
            tx.update(Collection,data.context._id,modifier,options,setStatus);
            EditableText._callback.call('afterUpdate',data.context);
            if (data.transactionUpdateText || data.objectTypeText) {
              tx.commit();
            }
          }
            ...
            data.context = EditableText._callback.call('beforeUpdate',data.context) || data.context;
            if (c2optionsHashRequired) {
              Collection.update({_id:data.context._id},modifier,EditableText.collection2options,setStatus,setStatus);
            }
            else {
