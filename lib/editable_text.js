...
EditableText.useMethods = true;

EditableText.insert = function(Collection,doc) {
  if (EditableText.useMethods && this.useMethod !== false || this.useMethod) {
    var self = this;
	var objId = new Meteor.Collection.ObjectID()._str;
	this.context._id = objId;
	Meteor.call('editableTextWrite','insert',this,null,EditableText._useTransaction(this),function(err,res) { // res is new doc _id
	  if (self.onAutoInsert && _.isString(self.onAutoInsert)) {
  ...
  // see above
  Tracker.flush();
  $('.et-' + res).eq(0).trigger((EditableText.wysiwyg && this.wysiwyg) ? 'mousedown' : 'click');
  return new_id;
}
