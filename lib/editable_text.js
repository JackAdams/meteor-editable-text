...
EditableText.insert = function (Collection, doc) {
  ...
  if (EditableText.useMethods && this.useMethod !== false || this.useMethod) {
    ...
    var openAfterAutoInsertTarget = (this.openAfterAutoInsertTarget) ? this.openAfterAutoInsertTarget + ' ' : '';
	var elem = $(openAfterAutoInsertTarget + '.et-' + objId + '-' + EditableText._removeNesting(this.field));
    if (elem.length) {
      elem.eq(0).trigger((EditableText.wysiwyg && self.wysiwyg) ? 'mousedown' : 'click');
    }
    return;
  ...
  // see above
  Tracker.flush();
  $('.et-' + new_id + '-' + EditableText._removeNesting(this.field)).eq(0).trigger((EditableText.wysiwyg && this.wysiwyg) ? 'mousedown' : 'click');
  return new_id;
}
...
EditableText._removeNesting = function (field) {
  return (field.indexOf('.') >= 0) ? field.replace(/\./g, '-') : field;
}
...
EditableText._makeUpdate = function (value, isEscape, evtType) {
  ...
  switch (type) {
    case 'number' :
      if (value !== '') {
        var rawNewValue = (this.allowNegative && _.isNumber(parseInt(value))) ? value : EditableText._extractNumber(value);
        var newValue =  (rawNewValue && _.isNumber(parseInt(rawNewValue))) ? parseInt(rawNewValue) : 0;
        keepZero = true;
      }
      else {
        var newValue = 0;
...
Template.editable_text_widget.helpers({
  ...
  wysiwygContent : function () {
    if (this.afterRender) {
      ...
      if (!tmpl.selected.get()) {
        var self = this;
        Tracker.afterFlush(function () {
          EditableText._callbacks[self.afterRender].call(self, self.context, Mongo.Collection.get(self.collection), tmpl);
        });
      }
    }
  ...
  removeNesting : function (field) {
    return EditableText._removeNesting(field);
  }
...
Template.editable_text_widget.onCreated(function () {
