...
EditableText.editing_key_press = function(elem) {
  if (EditableText.editing_key_press.fakeEl === undefined) {
    EditableText.editing_key_press.fakeEl = $('<span>').hide().appendTo(document.body);
  }
  var el = $(elem);
  EditableText.editing_key_press.fakeEl.text(el.val());
  el.width(EditableText.editing_key_press.fakeEl.width() + 20);
}
...
Template.editable_text_widget.events({
  'keyup .wide-text-edit' : function(evt) {
	if (this.autoResize) {
	  EditableText.editing_key_press(evt.target);	
	}
  }
});
    ...
    if (obj && !tmpl.selected.get()) {
      if (EditableText.userCanEdit.call(this,obj,Collection)) {
        ...
        if (!wysiwyg) {
		  ...
		  if (this.autoResize && !textarea) {console.log(this.field);
			EditableText.editing_key_press(tmpl.$('.wide-text-edit'));
		  }
        }
      }
    }
