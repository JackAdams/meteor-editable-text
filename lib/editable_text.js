...
EditableText.update = function(Collection,doc,modifier,callbacks,transactionText) {
  var self = this;
  if (EditableText.useMethods && this.useMethod !== false || this.useMethod) {
    Meteor.call('_editableTextWrite','update',this,modifier,EditableText._useTransaction(this),{callbacks:callbacks,transactionText:transactionText},function(err,res) {
      if (err) {
        console.log(err);
      }
    });
...
EditableText._makeUpdate = function(value,isEscape,evtType) {
  ...
  if (newValue !== currentValue || (!newValue && this.removeEmpty && !(this.acceptEmpty && evtType === 'focusout'))) {
    if (!newValue && this.removeEmpty && !(this.acceptEmpty && evtType === 'focusout')) {
    ...
    else if (!isEscape && (newValue !== currentValue) && (newValue || this.acceptEmpty || (this.wysiwyg && this.acceptEmpty !== false))) { // wysiwyg accepts empty by default unless explicitly told not to
      // Make the update
...
// Stolen from gwendall:body-events
// To handle situation with iron router
// TODO -- make gwendall:body-events a dependency and remove this (if he removes the underscore string dependency from the original)
